...
public class ProductServiceImpl implements ProductService {
    ...
    public ProductExecution addProduct(Product product, ImageHolder thumnail, List<ImageHolder> productImgHolderList) throws ProductOperationException {
        //空值判断
    ...
    @Transactional
    //1.若缩略图参数有值，则处理缩略图
    //若原先存在缩略图则先删除再添加新图，之后获取缩略图相对路径并复制给product
    //2.若商品详情图列表参数优质，对商品详情图片列表进行同样的操作
    ...
    * */
    ...
    private void addProductImgList(Product product,List<ImageHolder> productImgHolderList){
        //获取图片存储路径，这里直接存放到响应电偶文件夹底下
        String dest=PathUtil.getShopImagePath(product.getShop().getShopId());
        List<ProductImg> productImgList=new ArrayList<ProductImg>();
        //遍历
        for (ImageHolder productImgHolder:productImgHolderList){
            String imgAddr=ImageUtil.generatrNormalImg(productImgHolder,dest);
